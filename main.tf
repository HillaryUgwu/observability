...
resource "aws_instance" "controller" {
  ...
  user_data               = file("bin/bootstrap_controller")
  ...
  #  inline = [ "echo '${[for i in aws_instance.node:i.private_ip]}' >> inventory.ini" ]
  #}

  #provisioner "file" {
  #  content     = templatefile("${path.module}/src/inventory.ini", { ip_addrs = [self.private_ip] })
  #  destination = "/path/on/remote/instance/inventory.ini"
  #}
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      #"chmod +x /path/on/remote/instance/inventory.ini",
      "ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -i inventory.ini ansible/playbook.yml -e 'ansible_ssh_private_key_file=/home/ec2-user/id_rsa'"
    ]
  }
...
#resource "aws_security_group" "node_sg_old" {
# name        = "node-sg"
